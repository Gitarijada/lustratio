        ...
        <div class="col">
                    ...
                    <th scope="col">
                        <div id="div_select" style="border: 2px solid lightgray;">
                            ...
                            <select id="location_filter" name="location_filter" class="form-control">
                                <option selected value="">Izaberi Lokaciju</option>
                                @foreach($locations as $item)
                                    @if($item->id == $drop_item_selected->get('location_ID')) 
                                        <option value="{{ $item->id }}" selected>{{ $item->name }}</option>
                                    @else  
                                        <option value="{{ $item->id }}">{{ $item->name }}</option>
                                    @endif
            ...
            @foreach($valetudinarians as $valetudinarian)
                <tr>
                    ...
                    <td>{{ $valetudinarian->occupation }}</td>
                    <td>{{ $valetudinarian->position }}</td>
                    ...
                    <td>
                        @if (auth()->user() && isset(Auth::user()->id))
                            @if ((Auth::user()->id == $valetudinarian->owner_id) || Auth::user()->id == 1)
                                ...
                                <a href="{{ url('/edit/'.$valetudinarian->id.'?page='.$valetudinarian->currentPage()) }}" class="btn btn-sm btn-info">Edit</a>
                            @endif

                            @if (Auth::user()->id == 1)
                                <a href="{{ url('/destroy/'.$valetudinarian->id.'?page='.$valetudinarian->currentPage()) }}" class="btn btn-sm btn-danger">Delete</a>
                    ...
                    for(var key in data.valetudinarians) {
                            ...
                            html += '<tr><td><a href="' + SITEURL + '/show/' + data.valetudinarians[key].id + '#photo-section">' + 
                                '<img src="' + SITEURL + '/storage/vale_images/' + data.valetudinarians[key].image_name + '" class="w-4" alt="' + data.valetudinarians[key].image_name + '"></a>' + 
                                        ...
                                        '</td><td>' + data.valetudinarians[key].occupation +
                                        '</td><td>' + data.valetudinarians[key].position +
                                        '</td><td>' + data.valetudinarians[key].party_name + 
                                        '</td><td>' + data.valetudinarians[key].location_name;
